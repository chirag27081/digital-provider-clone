...
serve(async (req) => {
  ...
  try {
    ...
    // Get recent orders count and spending
    const { data: orderStats, error: statsError } = await supabaseClient
      .from('orders')
    ...
    const totalSpent = orderStats?.reduce((sum, order) => sum + parseFloat(order.total_cost), 0) || 0;
